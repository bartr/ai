...
This document defines the `PRD_QUALITY_REPORT_V1` payload emitted when the skill runs a PRD quality assessment. One payload is produced per Validate-exit, Finalize-exit, Finalize drift, or on-request PRD quality run in the same invocation as the paired `PRD_STANDARD_FINDINGS_V1` payload.
